...
from PyQt5.QtWidgets import (
    ...
    QHeaderView, QSizePolicy, QScrollArea, QSplitter
)
...
"""
    ...
    def update_position(self, lat, lon, has_fix=False):
        ...
        if self.web:
            js = f"updatePosition({lat}, {lon}, {'true' if has_fix else 'false'})"
...
class GPSWidget(QFrame):
    def __init__(self):
        ...
        self.stale_indicator.setStyleSheet("color: #ff6b6b; font-size: 9px; font-weight: bold;")
...
class DashboardWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        left_layout.addWidget(self.boot_status)
        ...
        multiview_btn.setStyleSheet("background-color: #16a085; border-radius: 8px; padding: 10px;")
        ...
        imu_widget = QWidget()
        imu_layout = QVBoxLayout(imu_widget)
        self.accel_plot = RealtimePlot("Acceleration", "g", 3, ['#ff6b6b', '#4ecdc4', '#ffe66d'], legend_names=['X', 'Y', 'Z'])
        self.gyro_plot = RealtimePlot("Gyroscope", "deg/s", 3, ['#ff6b6b', '#4ecdc4', '#ffe66d'], legend_names=['X', 'Y', 'Z'])
        ...
        # Orientation tab with attitude indicator + plot
        orient_widget = QWidget()
        orient_layout = QHBoxLayout(orient_widget)
        self.attitude_indicator = AttitudeIndicator()
        self.attitude_indicator.setMinimumSize(250, 250)
        orient_layout.addWidget(self.attitude_indicator)
        self.orient_plot = RealtimePlot("Orientation", "deg", 3, ['#a855f7', '#06b6d4', '#ffe66d'], legend_names=['Pitch', 'Roll', 'Yaw'])
        orient_layout.addWidget(PlotContainer(self.orient_plot, "Orientation", self))
        plots_tabs.addTab(orient_widget, "Orientation")
        ...
        # GPS Map tab
        if HAS_WEBENGINE:
            gps_map_widget = QWidget()
            gps_map_layout = QVBoxLayout(gps_map_widget)
            self.gps_map = GPSMapWidget()
            gps_map_layout.addWidget(self.gps_map)
            plots_tabs.addTab(gps_map_widget, "GPS Map")
        else:
            self.gps_map = None
        ...
        self.rocket_view = RocketView3D()
        self.rocket_view.setMinimumHeight(280)
